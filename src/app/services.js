...
angular.module('instastore')
    ...
    .service('SStorage', ['messageService', 'pouchDB',
      function (messageService, pouchDB) {
        this.isSessionStorageAvailable = function () {
          try {
            var db = pouchDB('store', {adapter: 'websql'});
            // db.put({
            //   _id: 'profile-store',
            //   address: 'hello'
            // }).then(function () {
            ...
            var user = {address: 'hello'};

            db.get('user').then(function (doc) {
              console.log(doc);
              db.put({user: user, _id: 'user', _rev: doc._rev})
                  .catch(messageService.simpleAlert('lstorageisnotavailable'));
            }).catch(function (err) {
              if (err.status === 404) {
                db.put({user: user, _id: 'user'}).then(function (res) {
                  console.log(res);
                }).catch(messageService.simpleAlert('lstorageisnotavailable'));
              }
            });
